<template>
    ...
    >
        <el-form :model="formData" :rules="rules" ref="vehicleRegisterUpdate" label-width="160px" status-icon
                 v-loading="loading">
            <el-row :gutter="20">
                <el-col :span="12">
        ...
        <span slot="footer" class="dialog-footer">
            <el-button @click="resetForm('vehicleRegisterUpdate')" icon="fa fa-undo">កំណត់ឡើងវិញ</el-button>
            <el-button type="primary" @click="submitForm('vehicleRegisterUpdate')" icon="fa fa-save" :loading="loading">រក្សាទុក</el-button>
        </span>
    </el-dialog>
</template>
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.loading = true;
            setTimeout(() => this.updateVehicleRegister(), 500);
          } else {
            return false;
          }
        });
      ...
      updateVehicleRegister() {
        Meteor.call('vehicle_updateRegister', {
      ...
      handleClose() {
        this.$emit('close');
        this.$emit('reset');
        this.resetForm('vehicleRegisterUpdate');
      },
    },
    watch: {
      currentRow(nextVal) {
